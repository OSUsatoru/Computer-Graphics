...
const char *WINDOWTITLE = { "Project2 -- Satoru Yamamoto" };
...
GLuint	BladeList;			// heli blade display list
GLuint	SphereList;			// sphere display list
float	BladeAngle;
int		view_int;				// inside or outside
bool		Frozen;
...
Animate( )
{
	...
	// const int MS_IN_THE_ANIMATION_CYCLE = 10000; // Is this miliseconds? check
	const int MS_IN_THE_ANIMATION_CYCLE = 4000;	// milliseconds in the animation loop
	...
	//define BladeAngle (check)
	BladeAngle = Time*360*2;
...
Display( )
{
	...
	// set Outside and Inside view (check)
	// A good eye poition for your Inside View is (-0.4, 1.8, -4.9) . 
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., (char *)"Project2: Satoru Yamamoto" );
...
InitMenus( )
{
	...
	int viewmodemenu = glutCreateMenu(DoViewMode);
	glutAddMenuEntry("Outside", Outside);
	glutAddMenuEntry("Inside", Inside);

	int mainmenu = glutCreateMenu( DoMainMenu );
	glutAddSubMenu("View", viewmodemenu);
