...
const int NUMPOINTS = 60;	// 10 is not enough, 100 is too many


struct Point
{
	float x0, y0, z0;       // initial coordinates
	float x, y, z;        // animated coordinates (need to change this )

	void setPoint(float xinput, float yinput, float zinput) {
		x = xinput;
		y = yinput;
		z = zinput;
		x0 = xinput;
...
void DrawingBezierCurve(struct Curve* curve, bool isCurve);
...
Display()
{
	...
	// workspace for project6 (check)
	glPushMatrix();
	float degree = 45;
	for (int i = 0; i < NUMCURVES; ++i) {
		// rotate z
		// time (0 - 1)
		// sin or cos(2*M_PI*time) -> (0-1)
		
		RotateZ(&Curves[i].p0, cos(2 * M_PI * Time) * degree, 0,0,0);
		RotateZ(&Curves[i].p1, -cos(2 * M_PI * Time) * degree, 0, 0, 0);
		RotateZ(&Curves[i].p2, -cos(2 * M_PI * Time) * degree, 0, 0, 0);
		RotateZ(&Curves[i].p3, cos(2 * M_PI * Time) * degree, 0, 0, 0);
		DrawingBezierCurve(&Curves[i], 1);
	}
	glPopMatrix();

	glPushMatrix();
	for (int i = 0; i < NUMCURVES; ++i) {
		DrawingBezierCurve(&Stems[i], 0);
	}
	glPopMatrix();
...
// check
void	DoPointsMenu(int id) {
	ControlpointsOn = id;
	glutSetWindow(MainWindow);
	glutPostRedisplay();
}

void	DoLinesMenu(int id) {
	ControlLinesOn = id;
	glutSetWindow(MainWindow);
	glutPostRedisplay();
}
...
InitGraphics()
{
	...
	// define Curve Curves[NUMCURVES];
	// x = rcos(theta), y = rsin(theta)
	// theta = 2*M_PI/NUMCURVES
	// x = rcos(theta + theta*i), y = rsin(theta + theta*i)

	float O_radius = 4.;
	float I_radius = 1.;
	float theta = 2 * M_PI / NUMCURVES;

	float div = O_radius / I_radius;

	for (int i = 0; i < NUMCURVES; ++i) {
		// base point is theta*i
		Curves[i].p0.setPoint(I_radius * sinf(theta * i + theta), I_radius * cosf(theta * i + theta), 0);
		// change z value to make 3D
		Curves[i].p1.setPoint(O_radius * sinf(theta * i + theta / 2), O_radius * cosf(theta * i + theta / 2), 2);
		Curves[i].p2.setPoint(O_radius * sinf(theta * i - theta / 2), O_radius * cosf(theta * i - theta / 2), 2);
		Curves[i].p3.setPoint(I_radius * sinf(theta * i - theta), I_radius * cosf(theta * i - theta), 0);

		// make 3 different 
		Curves[i].r = Colors[i % 8][i % 3];
		Curves[i].g = Colors[(i+1) % 8][(i+1) % 3];
		Curves[i].b = Colors[(i+2) % 8][(i+2) % 3];
	}

	// define Stems[NUMCURVES]
	// x = -2, y = -2, z = -2
	// -1 -1 -1 
	// 0 0 0 
	// p1
	for (int i = 0; i < NUMCURVES; ++i) {
		Stems[i].p0.setPoint(-2,-2,-2);
		Stems[i].p1.setPoint(-1,-1,-1);
		Stems[i].p2.setPoint(0,0,0);
		Stems[i].p3.setPoint(I_radius * sinf(theta * i - theta), I_radius * cosf(theta * i - theta), 0);
...
void DrawingBezierCurve(struct Curve* curve, bool isCurve) {
	...
	for (int it = 0; it <= NUMPOINTS; it++)
	{
		...
		if(isCurve)
			glColor3f(x + 1/ NUMPOINTS, y+1/ NUMPOINTS, z+1/ NUMPOINTS);
		glVertex3f(x, y, z);
	}
	glEnd();
	glLineWidth(1.);
