...
GLuint	SphereList;
GLSLProgram* Pattern;
bool ImUsingTheVertexShader;
bool ImUsingTheFragmentShader;
...
Display()
{
	...
	// object1: with texture / stationary (FLAT)
	glPushMatrix();
	...
	SetMaterial(0.5, 0.5, 0.5, 20.);
	OsuSphere(0.25, 100, 100);
	glDisable(GL_TEXTURE_2D);
	//glEnable(GL_LIGHTING);
	glPopMatrix();


	// object 2: Smooth / stationary / shiny
	glPushMatrix();
	//glDisable(GL_LIGHTING);
	glShadeModel(GL_SMOOTH);
	// ask
	SetMaterial(0., 1., 0., 300.);
	glColor3f(0., 1, 0);
	...
	// object 3: Flat / moving / Dull
	glPushMatrix();
	//glDisable(GL_LIGHTING);
	glShadeModel(GL_FLAT);
	// ask
	SetMaterial(1., 1., 0., 20.);
	glColor3f(1, 1, 0);
	glTranslatef(0., 0., 2.5 * sin(Time * M_PI * 2));
	//glutSolidCube(.25);
	OsuSphere(0.25 / 2, 100, 100);
	//glEnable(GL_LIGHTING);
	glPopMatrix();

	//------------- light objects ---------------

	// light 1: with solid cube object
	SetPointLight(GL_LIGHT0, 0, 0., -(0.25 / 2 + 0.05) + 2.5 * sin(Time * M_PI * 2), 0, 0, 1);
	glPushMatrix();
	glDisable(GL_LIGHTING);
	glTranslatef(0., 0., -(0.25 / 2 + 0.05) + 2.5 * sin(Time * M_PI * 2));
	glColor3f(0, 0, 1.);
	...
	// light 2: spotlight to Torus
	SetSpotLight(GL_LIGHT2, 0, -0.75, 0.75, 0., 0, -1, 1, 0, 0);
	glPushMatrix();
	glDisable(GL_LIGHTING);
	...
	// light 3: while point light

	SetPointLight(GL_LIGHT1, 0, 0.75, 0.75, 1, 1, 1);
	glPushMatrix();
	glDisable(GL_LIGHTING);
	glTranslatef(0, 0.75, 0.75);
	glColor3f(1, 1, 1.);
	...
	// switch lights (this comes after creating s)
	if (Light0On)
		glEnable(GL_LIGHT0);
	else
		glDisable(GL_LIGHT0);
	...
	*/
	...
	Pattern->SetUniformVariable( "uImUsingTheVertexShader",  ImUsingTheVertexShader );
	Pattern->SetUniformVariable( "uImUsingTheFragmentShader",  ImUsingTheFragmentShader );
	Pattern->SetUniformVariable( "uR",  R );
...
// check
void DoDistortions(int id) {
	if (id == NOTEXT) {
		Distoron = 0;
		Textureon = 0;
	}
	else if (id == NODISTOTEXT) {
		Distoron = 0;
		Textureon = 1;
	}
	else if (id == DISTOTEXT) {
		Distoron = 1;
		Textureon = 1;
	}
	else if (id == DISTOTEXT2) {
		Distoron = 2;
		Textureon = 1;
	}
	else if (id == DISTOTEXT3) {
		Distoron = 3;
		Textureon = 1;
	}
	glutSetWindow(MainWindow);
...
InitMenus()
{
	...
	int distormenu = glutCreateMenu(DoDistortions);
	glutAddMenuEntry("No Texture", 0);
	glutAddMenuEntry("Texture / No Distortion", 1);
	glutAddMenuEntry("Texture / Distortion1", 2);
	glutAddMenuEntry("Texture / Distortion2", 3);
	glutAddMenuEntry("Texture / Distortion3(original)", 4);
	...
	glutAddSubMenu("Distortions", distormenu);
...
InitGraphics()
{
	...
	// set up for texture
	int width, height;
	unsigned char* texture1 = BmpToTexture("worldtex.bmp", &width, &height);
...
Keyboard(unsigned char c, int x, int y)
{
	...
	{
	case 'b':
		ImUsingTheVertexShader = 1;
		ImUsingTheFragmentShader = 1;
		break;
	case 'f':
		ImUsingTheVertexShader = 0;
		ImUsingTheFragmentShader = 0;
		break;
	case 'F':
		ImUsingTheVertexShader = 0;
		ImUsingTheFragmentShader = 1;
...
OsuSphere(float radius, int slices, int stacks)
{
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
											...
											// ilng=NumLngs-1, lng=+M_PI are the same meridian
		{
			...
			// This equation is given by my friend who asked this equation to Teacher assistant.
			if (Distoron == 1) {
				//p->s = (lng + (M_PI / 4) * sin(Time * 2. * M_PI + 2. * lat) + M_PI) / (2. * M_PI);
				p->s = (lng + M_PI) / (2. * M_PI);
				p->t = (lng + (M_PI / 4) * sin(Time * 2. * M_PI + 2. * lat) + M_PI) / (2. * M_PI);
				//p->t = (lat + M_PI / 2.) / M_PI;
			}
			else if (Distoron == 2) {
				p->s = (lng + (M_PI / 4) * sin(Time * 2. * M_PI + 2. * lat) + M_PI) / (2. * M_PI);
				...
				p->s = (lng + M_PI) * sin(Time * M_PI * lat) / (2. * M_PI);
				p->t = (lat + M_PI / 2.) / M_PI;
			}
			else {
				p->s = (lng + M_PI) / (2. * M_PI);
