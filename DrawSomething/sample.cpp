...
InitLists( )
{
	...
	// (numsegs*num_layer) x-y-z coordinate
	int numsegs = 10;
	float radius = 0;
	int layer = 1000;
	//float dz = 0;
	...
	//glColor3f(rand_R, rand_G, rand_B);
	...
	for (int j = 0; j < layer; ++j) {
		...
		{
			glVertex3f(radius * cos(ang), radius * sin(ang), j*0.001);
			ang += dang;
			//printf("ang: %f\n", ang);
		}
		glEnd();
	...
	for (int i = 0; i < numsegs; i++)
	{
		...
		glVertex3f(radius * cos(ang) * dz, radius * sin(ang) * dz, dz);
		ang += dang;
